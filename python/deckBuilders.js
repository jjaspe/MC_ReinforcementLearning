...
import { BaseHandBuilder, HAND_BUILDER_TYPES } from './GameElements/handBuilders.js'

class BaseDeckBuilder {
  static makeDeckBuilder = function(deckBuilderType) {
    var deckBuilder = null
    var handBuilder = BaseHandBuilder.createHandBuilder(HAND_BUILDER_TYPES.RANDOM)
    switch (deckBuilderType) {
      ...
      case DECK_BUILDER_TYPES.DAMAGE_AND_CUSTOM_COST_CUSTOM_HAND:
        deckBuilder = new DamageAndCustomCostDeckBuilder(config.MIN_HERO_ALLY_ATTACK, config.MAX_HERO_ALLY_ATTACK,
          config.MIN_HERO_ALLY_COST, config.MAX_HERO_ALLY_COST)
          handBuilder = BaseHandBuilder.createHandBuilder(HAND_BUILDER_TYPES.ONE_OF_EACH)
          break;
      default:
